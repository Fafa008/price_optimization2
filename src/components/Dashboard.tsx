...
export function Dashboard() {
  ...
  const loadSummary = async () => {
    try {
      ...
      const totalRevenue = priceData?.reduce((sum, record) => sum + (record.total_price || 0), 0) || 0;
      const avgPrice = priceData && priceData.length > 0
        ? priceData.reduce((sum, record) => sum + (record.unit_price || 0), 0) / priceData.length
        : 0;
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1">Overview of your retail pricing data</p>
      </div>
